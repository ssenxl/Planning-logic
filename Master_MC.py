...
def load_all_master_mc(file_path: Path) -> pd.DataFrame:
    ...
    for sheet in xls.sheet_names:
        try:
            ...
            if "Capability_Group" not in df.columns:
                if "Capability Group" in df.columns:
                    df["Capability_Group"] = df["Capability Group"]
                elif "Group" in df.columns:
                    df["Capability_Group"] = df["Group"]
...
# CORE FUNCTION (FINAL)
# =========================
def get_mc_names_by_mc_and_guage(mc: str, guage: str, df: pd.DataFrame) -> dict:
    """
    Input: MC + Guage
    ...
    rows = df[(df["MC"] == mc) & (df["Guage"] == guage)]

    if rows.empty:
        raise ValueError(f"❌ Not found MC='{mc}' with Guage='{guage}'")

    cap_groups = rows["Capability_Group"].unique().tolist()

    result = {}
    for cap in cap_groups:
        mc_names = df[df["Capability_Group"] == cap]["MC"].unique().tolist()
...
if __name__ == "__main__":
    ...
    EXCEL_FILE = BASE_DIR / "data" / "MC" / "Master_MC_5.xlsx"
